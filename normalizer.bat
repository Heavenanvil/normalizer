...
:: Меняем кодировку на UTF-8, для совместимости с кириллическими именами файлов.
chcp 65001
...
ffmpeg -i %file%.%format% -y -map 0:v -c copy %file%+.%format%

:: Шаг 2 - Извлекаем первую аудиодорожку
:: ffmpeg -i %file%.%format% -y -c:a copy %file%.%aformat%
ffmpeg -i %file%.%format% -y -map 0:a -c copy %file%.%aformat%
...
ffmpeg -i %file%+.%format% -i %file%+++++.%aformat% -y -c:v copy -c:a copy -map 0:v:0 -map 1:a:0 %file%_norm.%format%
